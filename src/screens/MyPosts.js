...
export default function MyPosts() {
    ...
    console.log(allUserPosts , "all user posts")

    useEffect(async () => {
        var userId = user[0].id
        await API.getTopUserPosts(userId)
            .then(async res => setAllUserPosts(await res.data))
    }, [])

    const deleteUserPost = async e => {
        // e.preventDefault();
        console.log(e ,"keeeey")
    ...
    return (
        <div className="myPostsCont container">
                         {allUserPosts.map((posts, myKey) => (
            <span className="makeRow">
                     {/* <div className="idk"> */}
                    <div className="blogHolder">
                        <img src={posts.blog_img} alt={"your post"}></img>
                        <span className="makeColumn">
                            <button style={{backgroundImage : `url("${photo}")`}} id={posts.id} onClick={e  => {deleteUserPost(e.target.id)}}></button>
